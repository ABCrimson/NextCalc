...
- **pnpm** 11+ (packageManager field specifies `pnpm@11.0.0-alpha.11`)
...
GITHUB_CLIENT_ID="..."        # GITHUB_ID is accepted as a fallback
GITHUB_CLIENT_SECRET="..."    # GITHUB_SECRET is accepted as a fallback
...
pnpm --filter @nextcalc/web lint:fix   # Auto-fix lint issues (per-package; no root lint:fix)
```
...
GraphQL TypeScript types are generated by `graphql-codegen` using the config at
`apps/web/codegen.ts`. There is no `pnpm codegen` npm script — run the codegen CLI
directly (e.g. via `npx graphql-codegen --config apps/web/codegen.ts`) when the
schema or operations change.
