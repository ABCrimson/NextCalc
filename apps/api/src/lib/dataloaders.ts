...
import type { PrismaClient, User, Folder, WorksheetShare } from '@nextcalc/database';
...
export function createDataLoaders(prisma: PrismaClient): DataLoaders {
  return {
    ...
    worksheetSharesByWorksheetId: new DataLoader<string, WorksheetShare[]>(
      async (worksheetIds) => {
    ...
    childFoldersByParentId: new DataLoader<string, Folder[]>(
      async (parentIds) => {
        const folders = await prisma.folder.findMany({
    ...
    upvoteCountByTargetId: new DataLoader<string, number>(
      async (targetIds) => {
