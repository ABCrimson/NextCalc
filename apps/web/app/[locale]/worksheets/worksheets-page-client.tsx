...
/**
 * My Worksheets — interactive client shell.
 *
 * Receives the initial worksheet list and session user from the Server
 * Component page; handles search, sort, view-mode, and delete flows locally.
 */
...
export interface WorksheetsUser {
  id: string;
  name: string | null;
}

export interface WorksheetsPageClientProps {
  /** Worksheets loaded on the server for the current user */
  initialWorksheets: WorksheetSummary[];
  /** Session user (null when signed out) */
  user: WorksheetsUser | null;
}
...
export function WorksheetsPageClient({ initialWorksheets, user }: WorksheetsPageClientProps) {
  const t = useTranslations('worksheets');
  const tAuth = useTranslations('auth');
  const format = useFormatter();

  const [worksheets, setWorksheets] = useState<WorksheetSummary[]>(initialWorksheets);
    ...
    <main className="min-h-screen px-4 py-8 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      {!user && (
        <div className="text-center py-20">
          <LogIn className="size-12 text-muted-foreground mx-auto mb-4 opacity-50" />
          <h2 className="text-lg font-medium text-foreground mb-2">{tAuth('signInRequired')}</h2>
          <p className="text-muted-foreground text-sm mb-6">{tAuth('signInSubtitle')}</p>
          <Link
            href="/auth/signin"
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-primary-foreground font-medium text-sm hover:bg-primary/90 transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"
          >
