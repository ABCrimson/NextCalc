...
import { Flame, Info, Pause, Play, Radio, RotateCcw, Settings, Sparkles, Zap } from 'lucide-react';
  ...
  for (let i = 1; i < nx - 1; i++) {
    for (let j = 1; j < ny - 1; j++) {
      const laplacian =
        ...
        4 * (uCurr[i]?.[j] ?? 0);
      uNext[i]![j] =
        (uCurr[i]?.[j] ?? 0) + dt * (initialVelocity[i]?.[j] ?? 0) + 0.5 * r * laplacian;
    }
  }
...
export default function PDESolverPage() {
    ...
    (initialCondition: number[][]) => {
      ...
      try {
        ...
        if (equationType === 'heat') {
          ...
          const initialVelocity = Array(gridSize)
            .fill(0)
   ...
   */
  useEffect(() => {
    solvePDE(generateInitialCondition(initialCondition, gridSize));
  }, [solvePDE, initialCondition, gridSize]);
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground font-mono">
                    {equationType === 'heat' ? t('heatFormula') : t('waveFormula')}
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Slider
                    ...
                    className="py-2"
                  />
                  <p className="text-xs text-muted-foreground">{t('gpuHighRes')}</p>
                </div>
                ...
                {equationType === 'heat' ? (
                  <div className="space-y-2">
                    ...
                    <Slider
                      ...
                      className="py-2"
                    />
                    <p className="text-xs text-muted-foreground">{t('higherDiffusion')}</p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    ...
                    <Slider
                      ...
                      className="py-2"
                    />
                    <p className="text-xs text-muted-foreground">{t('propagationSpeed')}</p>
                  </div>
                )}
        ...
        <section className="mt-12 space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <div className="group relative p-6 rounded-lg bg-linear-to-br/oklab from-emerald-950/40 to-emerald-900/40 border border-emerald-500/40 hover:border-emerald-400/70 transition-all duration-300 backdrop-blur-sm">
              ...
              <p className="text-sm text-emerald-200/80">{t('applicationsAbout')}</p>
            </div>
          </div>
        </section>
      </div>
