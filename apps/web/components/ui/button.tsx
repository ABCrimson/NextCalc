...
function Button({ className, variant, size, asChild = false, ref, ...props }: ButtonProps) {
  ...
  return (
    ...
    >
      {props.children}
      <span
        className="absolute inset-0 -translate-x-full group-hover:translate-x-full transition-transform duration-700 bg-linear-to-r/oklab from-transparent via-white/10 to-transparent pointer-events-none"
        aria-hidden="true"
      />
    </button>
  );
