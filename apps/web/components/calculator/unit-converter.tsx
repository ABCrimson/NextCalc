...
import { AlertCircle, ArrowLeftRight, History, Search, TrendingUp, RotateCcw } from 'lucide-react';
...
const CATEGORIES: Record<CategoryKey, CategoryDef> = {
  ...
  temperature: {
    ...
    units: [
      ...
      { name: 'celsius',     symbol: '°C', toSI: 1,     preOffset: 273.15,  searchTerms: ['c', 'celsius', 'centigrade'] },
      // Fahrenheit: K = (F + 459.67) × 5/9
      { name: 'fahrenheit',  symbol: '°F', toSI: 5 / 9, preOffset: 459.67,  searchTerms: ['f', 'fahrenheit'] },
  ...
  volume: {
    ...
    units: [
      { name: 'milliliter',       symbol: 'mL',   toSI: 1e-6,          searchTerms: ['ml', 'millilitre', 'cc', 'cm3'] },
      { name: 'liter',            symbol: 'L',    toSI: 1e-3,          searchTerms: ['l', 'litre'] },
      { name: 'cubic meter',      symbol: 'm³',   toSI: 1,             searchTerms: ['m3', 'cu m'] },
      { name: 'fluid ounce (US)', symbol: 'fl oz',toSI: 2.95735e-5,    searchTerms: ['fl oz', 'floz', 'fluid ounce'] },
      ...
      { name: 'gallon (UK)',      symbol: 'Imp gal', toSI: 4.54609e-3, searchTerms: ['imperial gallon', 'uk gallon'] },
    ],
    commonConversions: [
  ...
  speed: {
    ...
    units: [
      { name: 'meter/second',   symbol: 'm/s',   toSI: 1,            searchTerms: ['m/s', 'mps', 'meters per second'] },
      ...
      { name: 'foot/second',    symbol: 'ft/s',  toSI: 0.3048,       searchTerms: ['ft/s', 'fps', 'feet per second'] },
      { name: 'Mach (sea lvl)', symbol: 'M',     toSI: 340.29,       searchTerms: ['mach', 'speed of sound'] },
    ],
    commonConversions: [
  ...
  pressure: {
    ...
    units: [
      ...
      { name: 'mmHg (Torr)',  symbol: 'mmHg', toSI: 133.322,    searchTerms: ['mmhg', 'torr', 'mm hg'] },
    ],
    commonConversions: [
  ...
  energy: {
    ...
    units: [
      ...
      { name: 'kilocalorie',symbol: 'kcal', toSI: 4184,        searchTerms: ['kcal', 'kilocalorie', 'food calorie'] },
      ...
      { name: 'electronvolt', symbol: 'eV', toSI: 1.60218e-19, searchTerms: ['ev', 'electronvolt'] },
  ...
  force: {
    ...
    units: [
      ...
      { name: 'pound-force', symbol: 'lbf', toSI: 4.44822,   searchTerms: ['lbf', 'pound force', 'lb-f'] },
  ...
  angle: {
    ...
    units: [
      { name: 'degree',     symbol: '°',    toSI: Math.PI / 180,        searchTerms: ['deg', 'degree', 'degrees'] },
      { name: 'radian',     symbol: 'rad',  toSI: 1,                    searchTerms: ['rad', 'radian'] },
      { name: 'gradian',    symbol: 'grad', toSI: Math.PI / 200,        searchTerms: ['grad', 'gon', 'gradian'] },
      { name: 'arcminute',  symbol: "'",    toSI: Math.PI / 10800,      searchTerms: ["arcmin", "arc minute", "'"] },
      { name: 'arcsecond',  symbol: '"',    toSI: Math.PI / 648000,     searchTerms: ['arcsec', 'arc second', '"'] },
      { name: 'revolution', symbol: 'rev',  toSI: 2 * Math.PI,          searchTerms: ['rev', 'revolution', 'turn'] },
    ],
    commonConversions: [
...
function buildFormula(from: UnitDef, to: UnitDef): string {
  ...
  if (hasPreOffset) {
    ...
    const toK = from.preOffset !== undefined
      ? `(x + ${from.preOffset}) × ${formatNum(from.toSI)}`
      : `x × ${formatNum(from.toSI)}`;
    const fromK = to.preOffset !== undefined
...
function CategoryChip({ categoryKey, category, isActive, onSelect }: CategoryChipProps) {
  return (
    ...
    >
      ...
      <span className="leading-none truncate w-full text-center" style={{ fontSize: category.label.length > 7 ? '0.6rem' : undefined }}>{category.label}</span>
    </button>
  );
}
...
function UnitSelect({
  ...
  return (
    <div className="space-y-2">
      ...
      >
        {filtered.length === 0 ? (
          <option disabled value="">No units match</option>
        ) : (
          filtered.map((u) => (
...
export function UnitConverter() {
    ...
    try {
      ...
      setRecentConversions((prev) => {
        const deduped = prev.filter(
          (c) => !(c.fromLabel === fromLabel && c.toLabel === toLabel)
        );
  ...
  useEffect(() => { runConversion(); }, [runConversion]);
        ...
        {result !== null && !error && (
          ...
          >
            ...
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              <Badge variant="outline" className="font-mono text-xs text-muted-foreground px-2 py-0.5">
                {formula}
              </Badge>
            </div>
          </div>
        ...
        <details className="text-sm">
          ...
          <ul className="mt-2 space-y-1 text-muted-foreground list-disc list-inside">
            {categoryDef.facts.map((fact) => (
              <li key={fact} className="text-xs">{fact}</li>
            ))}
          </ul>
        </details>
      </CardContent>
