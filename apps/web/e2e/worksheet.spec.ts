...
test.describe('Worksheet Page', () => {
  ...
  test('page loads successfully', async ({ page }) => {
    ...
    await expect(
      page.locator('main[aria-label*="worksheet" i]'),
  ...
  test('worksheet editor area is visible', async ({ page }) => {
    ...
    const editor = page.locator('[data-testid="worksheet-editor"], [role="textbox"], textarea, .worksheet-cell').first();
    // Give extra time since this is a dynamic import with ssr: false
    await expect(editor).toBeVisible({ timeout: 15000 }).catch(() => {
