...
import { solve, Complex, type Solution } from '../solver/solve';
...
function rootToLatex(value: number | { real: number; imag: number }, variable: string): string {
  ...
  const iStr = Number.isInteger(Math.abs(imag)) ? String(Math.abs(imag)) : Math.abs(imag).toFixed(4).replace(/\.?0+$/, '');
...
function nodeToLatex(node: ExpressionNode): string {
  ...
  if (isFunctionNode(node)) {
    ...
    switch (fn) {
      ...
      case 'Si': return `\\operatorname{Si}\\left(${args[0]}\\right)`;
      case 'Ci': return `\\operatorname{Ci}\\left(${args[0]}\\right)`;
      case 'li': return `\\operatorname{li}\\left(${args[0]}\\right)`;
      case 'erf': return `\\operatorname{erf}\\left(${args[0]}\\right)`;
...
export class StepSolver {
  ...
  private solveSimplification(problem: string): StepSolution {
    ...
    steps.push(this.createStep(
      expr, expr,
      'Analyze', `Expression type: ${analysis.type}`,
  ...
  private solveEquation(problem: string): StepSolution {
    ...
    if (!rhsIsZero) {
      steps.push(this.createStep(
        lhs, simplified,
        'Rearrange', 'Move all terms to the left side',
    ...
    steps.push(this.createStep(
      simplified, simplified,
      'Classify', this.equationTypeLabel(eqType),
  ...
  private solveLinearWithSteps(
    ...
    const xVal = -b / a;
    const xLatex = Number.isInteger(xVal)
      ? String(xVal)
      : `\\frac{${-b}}{${a}}`;

    steps.push(this.createStep(
      expr, createConstantNode(xVal),
      'Divide',
  ...
  private solveQuadraticWithSteps(
    ...
    const canFactor = discriminant >= 0 && Number.isInteger(Math.round(sqrtD * 1e6) / 1e6) &&
    ...
    if (canFactor) {
      ...
      const f2Latex = r1 === r2
        ? `^{2}`
        : `\\left(${variable} ${r2 <= 0 ? '+' : '-'} ${Math.abs(r2)}\\right)`;
      ...
      const solutions: Solution[] = r1 === r2
        ? [{ value: r1, multiplicity: 2 }]
        : [{ value: r1, multiplicity: 1 }, { value: r2, multiplicity: 1 }];
      return solutions;
    }
    ...
    if (discriminant >= 0) {
      ...
      return [{ value: r1exact, multiplicity: 1 }, { value: r2exact, multiplicity: 1 }];
    }
  ...
  private solveCubicWithSteps(
    ...
    const solutionLatex = solutions
      .map((s, i) => `${variable}_{${i + 1}} = ${formatDecimal(typeof s.value === 'number' ? s.value : s.value.real)}`)
      .join(', \\quad ');
    ...
    steps.push(this.createStep(
      ...
      solutions.map((s) => `${variable} = ${formatDecimal(typeof s.value === 'number' ? s.value : s.value.real)}`).join(', ') + '.',
      StepCategory.Evaluation,
      solutionLatex
    ));
  ...
  private solveTrigWithSteps(
    ...
    steps.push(this.createStep(
      ...
      `Remember the domain restriction: ${
        trigFn === 'sin' || trigFn === 'cos' ? '|k| ≤ 1 for real solutions' : 'k can be any real number'
      }.`,
      StepCategory.Isolation,
      `\\${trigFn}(${variable}) = k`
    ));
    ...
    const numLatex = solutions.length > 0
      ? solutions.map((s) => `${variable} = ${formatDecimal(typeof s.value === 'number' ? s.value : s.value.real)}`).join(', \\quad ')
  ...
  private solveDerivative(problem: string): StepSolution {
    ...
    if (!astEquals(derivative, simplified)) {
      steps.push(this.createStep(
        ...
        'Simplify', 'Simplify the derivative',
  ...
  private explainDerivativeRule(_expr: ExpressionNode, rule: string): string {
    const explanations: Record<string, string> = {
      ...
      'tan derivative rule': "The derivative of tan(u) is sec\u00B2(u) \u00B7 u', by the chain rule applied to sin(u)/cos(u).",
      'asin derivative rule': "The derivative of arcsin(u) is u'/\u221A(1 - u\u00B2), derived from implicit differentiation of y = arcsin(u).",
      'acos derivative rule': "The derivative of arccos(u) is -u'/\u221A(1 - u\u00B2), the negative of the arcsin derivative.",
      'atan derivative rule': "The derivative of arctan(u) is u'/(1 + u\u00B2), derived from implicit differentiation of y = arctan(u).",
      'sinh derivative rule': "The derivative of sinh(u) is cosh(u) \u00B7 u', since d/dx[sinh(x)] = cosh(x).",
      'cosh derivative rule': "The derivative of cosh(u) is sinh(u) \u00B7 u', since d/dx[cosh(x)] = sinh(x).",
      'tanh derivative rule': "The derivative of tanh(u) is sech\u00B2(u) \u00B7 u' = (1 - tanh\u00B2(u)) \u00B7 u'.",
