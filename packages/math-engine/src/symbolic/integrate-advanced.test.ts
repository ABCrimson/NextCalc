...
import { integrateAuto, integrateImproper } from './integrate-improper';
import {
  integrateDouble,
  integrateOverDisk,
  integrateOverSphere,
...
describe('Adaptive Simpson Integration', () => {
  it('efficiently integrates smooth functions', () => {
    // ∫₀^(π/2) cos(x) dx = 1
    const result = integrateAdaptiveSimpson(parse('cos(x)'), 'x', 0, Math.PI / 2, {
      tolerance: 1e-10,
    });
  ...
  it('adapts to non-smooth functions', () => {
    // ∫₀¹ sqrt(x) dx = 2/3
    const result = integrateAdaptiveSimpson(parse('sqrt(x)'), 'x', 0, 1, { tolerance: 1e-8 });
  ...
  it('handles rapidly varying functions', () => {
    ...
    const result = integrateAdaptiveSimpson(parse('sin(20*x)'), 'x', 0, 1, { tolerance: 1e-6 });
...
describe('Gauss-Kronrod Integration', () => {
  ...
  it('handles oscillatory functions well', () => {
    // ∫₀^(2π) sin²(x) dx = π
    const result = integrateGaussKronrod(parse('sin(x)^2'), 'x', 0, 2 * Math.PI, {
      tolerance: 1e-10,
    });
...
describe('Romberg Integration', () => {
  it('achieves exponential convergence for smooth functions', () => {
    // ∫₀¹ x⁴ dx = 1/5
    const result = integrateRomberg(parse('x^4'), 'x', 0, 1, { tolerance: 1e-12 });
  ...
  it('integrates periodic functions efficiently', () => {
    // ∫₀^(2π) cos(x) dx = 0
    const result = integrateRomberg(parse('cos(x)'), 'x', 0, 2 * Math.PI, { tolerance: 1e-10 });
...
describe('Monte Carlo Integration', () => {
  it('approximates integrals stochastically', () => {
    // ∫₀¹ x dx = 1/2
    const result = integrateMonteCarlo(parse('x'), 'x', 0, 1, { samples: 100000, tolerance: 1e-2 });
  ...
  it('handles multi-dimensional-like complexity', () => {
    // ∫₀¹ (x² + x³) dx = 1/3 + 1/4 = 7/12
    const result = integrateMonteCarlo(parse('x^2 + x^3'), 'x', 0, 1, { samples: 200000 });
...
describe('Improper Integrals', () => {
  it('handles infinite upper bound', () => {
    // ∫₁^∞ 1/x² dx = 1
    const result = integrateImproper(parse('1/x^2'), 'x', 1, Infinity, {
      infiniteStrategy: 'truncation',
      tolerance: 1e-6,
    });
  ...
  it('handles infinite lower bound', () => {
    // ∫₋∞⁰ e^x dx = 1
    const result = integrateImproper(parse('exp(x)'), 'x', -Infinity, 0, {
      infiniteStrategy: 'truncation',
      tolerance: 1e-6,
    });
  ...
  it('handles both infinite bounds', () => {
    // ∫₋∞^∞ e^(-x²) dx = √π
    const result = integrateImproper(parse('exp(-x^2)'), 'x', -Infinity, Infinity, {
      infiniteStrategy: 'truncation',
      tolerance: 1e-4,
    });
  ...
  it('detects and handles singularities', () => {
    ...
    const result = integrateAuto(parse('1/sqrt(x)'), 'x', 0, 1, {
      detectSingularities: true,
      tolerance: 1e-6,
    });
  ...
  it('handles logarithmic singularity', () => {
    // ∫₀¹ ln(x) dx = -1
    const result = integrateAuto(parse('log(x)'), 'x', 0, 1, {
      detectSingularities: true,
      tolerance: 1e-6,
    });
...
describe('Double Integrals', () => {
  it('integrates over rectangular domain', () => {
    // ∫₀¹ ∫₀¹ x*y dx dy = 1/4
    const result = integrateDouble(parse('x*y'), 'x', 0, 1, 'y', 0, 1, { tolerance: 1e-6 });
  ...
  it('handles variable bounds', () => {
    ...
    const result = integrateDouble(parse('y'), 'x', 0, 1, 'y', 0, (vars) => vars.x, {
      tolerance: 1e-6,
    });
  ...
  it('integrates polynomial functions', () => {
    // ∫₀¹ ∫₀¹ (x² + y²) dx dy = 2/3
    const result = integrateDouble(parse('x^2 + y^2'), 'x', 0, 1, 'y', 0, 1, { tolerance: 1e-6 });
...
describe('Triple Integrals', () => {
  it('integrates over box domain', () => {
    // ∫₀¹ ∫₀¹ ∫₀¹ x*y*z dx dy dz = 1/8
    const result = integrateTriple(parse('x*y*z'), 'x', 0, 1, 'y', 0, 1, 'z', 0, 1, {
      tolerance: 1e-4,
      subdivisions: 10,
    });
  ...
  it('computes volume of unit cube', () => {
    // ∫₀¹ ∫₀¹ ∫₀¹ 1 dx dy dz = 1
    const result = integrateTriple(parse('1'), 'x', 0, 1, 'y', 0, 1, 'z', 0, 1, {
      tolerance: 1e-4,
      subdivisions: 10,
    });
...
describe('Integration over Special Domains', () => {
  it('integrates over disk', () => {
    ...
    const result = integrateOverDisk(parse('1'), 'x', 'y', 1, {
      tolerance: 1e-4,
      subdivisions: 20,
    });
  ...
  it('computes volume of unit sphere', () => {
    // ∫∫∫_{x²+y²+z²≤1} 1 dV = 4π/3
    const result = integrateOverSphere(parse('1'), 'x', 'y', 'z', 1, {
      tolerance: 1e-2,
      subdivisions: 8,
...
describe('Property-Based Testing', () => {
    ...
    fc.assert(
      fc.property(
        ...
        (a, b) => {
          ...
          const relativeError =
            error / Math.max(Math.abs(resultCombined.value), Math.abs(expected), 1);
  ...
  it('reversal: ∫ₐᵇ f = -∫ᵦᵃ f', () => {
    fc.assert(
      fc.property(
        fc
          .double({ min: 0, max: 5, noNaN: true, noDefaultInfinity: true })
          .filter((x) => Math.abs(x) > 1e-100),
        fc
          .double({ min: 5, max: 10, noNaN: true, noDefaultInfinity: true })
          .filter((x) => Math.abs(x) > 1e-100),
        (a, b) => {
          // Guard against edge cases
          if (!Number.isFinite(a) || !Number.isFinite(b)) return true;
          ...
          const relativeError =
            error / Math.max(Math.abs(forward.value), Math.abs(backward.value), 1);
  ...
  it('additivity: ∫ₐᵇ f + ∫ᵦᶜ f = ∫ₐᶜ f', () => {
    fc.assert(
      fc.property(
        fc
          .double({ min: 0, max: 3, noNaN: true, noDefaultInfinity: true })
          .filter((x) => Math.abs(x) > 1e-100),
        fc
          .double({ min: 3, max: 6, noNaN: true, noDefaultInfinity: true })
          .filter((x) => Math.abs(x) > 1e-100),
        fc
          .double({ min: 6, max: 10, noNaN: true, noDefaultInfinity: true })
          .filter((x) => Math.abs(x) > 1e-100),
        (a, b, c) => {
          // Guard against edge cases
...
describe('Error Handling', () => {
  ...
  it('reports convergence issues', () => {
    const result = integrateNumerical(parse('sin(100*x)'), 'x', 0, 10, {
      tolerance: 1e-15,
      maxIterations: 10,
    });
...
describe('Performance Benchmarks', () => {
  it('completes smooth integral in < 100ms', () => {
    ...
    integrateNumerical(parse('x^3 + 2*x^2 - x + 1'), 'x', 0, 10, { tolerance: 1e-10 });
  ...
  it('handles complex expression efficiently', () => {
    ...
    integrateNumerical(parse('sin(x)*cos(x)*exp(-x/10)'), 'x', 0, 20, { tolerance: 1e-8 });
