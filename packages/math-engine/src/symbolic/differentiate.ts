    ...
    case '^':
      ...
      return createOperatorNode('*', 'multiply', [
        ...
        createOperatorNode('+', 'add', [
          // g' * ln(f)
          createOperatorNode('*', 'multiply', [rightPrime, createFunctionNode('log', [left])]),
          // g * f'/f
          createOperatorNode('*', 'multiply', [
            right,
...
function differentiateFunction(node: FunctionNode, variable: string): ExpressionNode {
  ...
  switch (node.fn) {
    case 'sin':
      // sin'(f) = cos(f) * f'
      return createOperatorNode('*', 'multiply', [createFunctionNode('cos', [arg]), argPrime]);
    ...
    case 'tan':
      ...
      return createOperatorNode('*', 'multiply', [
        createOperatorNode('/', 'divide', [
          createConstantNode(1),
          createOperatorNode('^', 'pow', [createFunctionNode('cos', [arg]), createConstantNode(2)]),
        ]),
        argPrime,
      ]);
    ...
    case 'exp':
      // exp'(f) = exp(f) * f'
      return createOperatorNode('*', 'multiply', [createFunctionNode('exp', [arg]), argPrime]);

    case 'log':
    case 'ln':
      // log'(f) = ln'(f) = (1/f) * f'
      return createOperatorNode('*', 'multiply', [
        createOperatorNode('/', 'divide', [createConstantNode(1), arg]),
        argPrime,
      ]);
    ...
    case 'sec':
      ...
      return createOperatorNode('*', 'multiply', [
        createOperatorNode('*', 'multiply', [
          createFunctionNode('sec', [arg]),
          createFunctionNode('tan', [arg]),
        ]),
        argPrime,
      ]);
    ...
    case 'csc':
      ...
      return createOperatorNode('*', 'multiply', [
        createOperatorNode('*', 'multiply', [
          createConstantNode(-1),
          createOperatorNode('*', 'multiply', [
            createFunctionNode('csc', [arg]),
            createFunctionNode('cot', [arg]),
          ]),
        ]),
        argPrime,
      ]);
    ...
    case 'cot':
      ...
      return createOperatorNode('*', 'multiply', [
        createOperatorNode('*', 'multiply', [
          createConstantNode(-1),
          createOperatorNode('^', 'pow', [createFunctionNode('csc', [arg]), createConstantNode(2)]),
        ]),
        argPrime,
      ]);
      ...
      return createOperatorNode('*', 'multiply', [
        createOperatorNode('/', 'divide', [
          ...
          createFunctionNode('sqrt', [
            createOperatorNode('-', 'subtract', [
              createConstantNode(1),
              createOperatorNode('^', 'pow', [arg, createConstantNode(2)]),
            ]),
          ]),
        ]),
        argPrime,
    ...
    case 'sinh':
      // sinh'(f) = cosh(f) * f'
      return createOperatorNode('*', 'multiply', [createFunctionNode('cosh', [arg]), argPrime]);

    case 'cosh':
      // cosh'(f) = sinh(f) * f'
      return createOperatorNode('*', 'multiply', [createFunctionNode('sinh', [arg]), argPrime]);
