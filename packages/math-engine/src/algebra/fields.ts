  ...
  for (const a of elements) {
    for (const b of elements) {
      if (multiply(a, b) !== multiply(b, a)) {
        ...
        errors.push(
          `Multiplication not commutative: ${String(a)} * ${String(b)} ≠ ${String(b)} * ${String(a)}`,
        );
        break;
      }
    }
...
export function createQuadraticExtensionOps(d: number): {
  readonly add: (
    x: QuadraticExtensionElement,
    y: QuadraticExtensionElement,
 ...
 */
export function findRootsFiniteField(
  poly: Polynomial,
  field: Field<number>,
...
export function galoisGroupFiniteField(
  ...
  for (let i = 0; i < n; i++) {
    automorphisms.push(`φ^${i}(x) = x^${p ** i}`);
  }
