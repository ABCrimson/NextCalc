 ...
 *
 * NOTE: This root entry point (`@nextcalc/math-engine`) re-exports the
 * following submodules only:
 *
 *   algebra, algorithms, complex, content, differential, fourier,
 *   graph-theory, knowledge, number-theory, parser, problems, prover,
 *   stats, symbolic
 *
 * The following submodules are intentionally NOT re-exported here (to keep
 * tree-shaking effective for consumers who only need one of them) and must
 * be imported from their subpath instead:
 *
 ...
 *   @nextcalc/math-engine/chaos
 *   @nextcalc/math-engine/game-theory
 *
 * For optimal tree-shaking, consumers should generally prefer subpath
 * imports even for the submodules re-exported here:
 *
 *   // Prefer this (only pulls in the parser module):
 *   import { parse } from '@nextcalc/math-engine/parser';
 *
 *   // Instead of this (pulls in everything re-exported below):
